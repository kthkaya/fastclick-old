...
E0_OUT  ::  Queue(500000) -> ToDevice($Name-eth0);

// eth1 interface - Inbound and Outbound
//------------------------------------------------------------
E1_IN  ::  FromDevice($Name-eth1, METHOD LINUX);
E1_OUT ::  Queue(500000) -> ToDevice($Name-eth1);

// Address Info container elements for the Interfaces
//------------------------------------------------------------
AddressInfo(E0_ADD $IP0/24 00:00:46:$MAC0);
AddressInfo(E1_ADD $IP1/24 00:00:46:$MAC1);
...
AR_E0  ::   ARPResponder(0.0.0.0/0 00:00:46:$MAC0) -> E0_OUT;
AR_E1  ::   ARPResponder(0.0.0.0/0 00:00:46:$MAC1) -> E1_OUT;
...
//--------------------------------------------------------------------------
E0_PKT_CLS  :: IPClassifier(dst host $IP0 and icmp type echo, icmp type echo, -)
E1_PKT_CLS  :: IPClassifier(dst host $IP1 and icmp type echo, icmp type echo-reply, -)
...
PING_RW :: ICMPPingRewriter(pattern $IP1 - 0-65535# 0 1, drop)
IP_RW   :: IPRewriter(pattern $IP1 1024-65535# - - 0 1, drop)
